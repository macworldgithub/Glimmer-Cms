...
import {
  ...
  getAllProductItem,
  createCategory,
  updateCategory, // Newly added import
  deleteCategory, // Newly added import
  createSubcategory,
  updateSubcategory,
  deleteSubcategory,
  createProductItem,
...
const CategoryTable = () => {
  ...
  const closeCategoryModal = async () => {
    if (modalType === "Add") {
      try {
        const res = await createCategory(newCategory, "aaaa", selectedCategory._id);
        if (res) {
    ...
    if (modalType === "Update") {
      try {
        ...
        if (res) {
          message.success(`Updated ${newCategory}`);
    ...
    if (modalType === "Delete") {
      try {
        const res = await deleteCategory(selectedCategory._id);
        if (res) {
          message.success(`Deleted ${selectedCategory.name}`);
          fetchCategories(); // Refresh categories after deleting
          setSelectedCategory({ _id: "", name: "" }); // Reset selection
          setFilterSubcategory([]); // Clear subcategories
          setSelectedSubcategory({ _id: "", name: "" }); // Reset subcategory selection
          setFilterProducts([]); // Clear products
          setSelectedProduct({ _id: "", name: "" }); // Reset product selection
        }
      } catch (error) {
        console.error("error", error);
  ...
  const closeSubCategoryModal = async () => {
    if (modalType === "Add") {
      try {
        ...
        if (res) {
          message.success("New Sub Category Added");
    ...
    if (modalType === "Update") {
      try {
        ...
        if (res) {
          message.success(`Updated ${newSubcategory}`);
    ...
    if (modalType === "Delete") {
      try {
        const res = await deleteSubcategory(selectedSubcategory._id);
        if (res) {
          message.success(`Deleted ${selectedSubcategory.name}`);
  ...
  const closeProductModal = async () => {
    if (modalType === "Add") {
      try {
        ...
        if (res) {
          ...
          fetchProductItems(); // Refresh product items after adding
        }
      } catch (error) {
        console.error("error", error);
    ...
    if (modalType === "Update") {
      try {
        ...
        if (res) {
          message.success(`Updated ${newProduct}`);
          fetchProductItems(); // Refresh product items after updating
        }
      } catch (error) {
        console.error("error", error);
    ...
    if (modalType === "Delete") {
      try {
        const res = await deleteProductItem(selectedProduct._id);
        if (res) {
          message.success(`Deleted ${selectedProduct.name}`);
          fetchProductItems(); // Refresh product items after deleting
          setSelectedProduct({ _id: "", name: "" }); // Reset product selection
        }
      } catch (error) {
        console.error("error", error);
    ...
    <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
      ...
      {selectedCategory?._id && (
        <div className="mt-4 flex gap-3 justify-center">
          <Button
            className="bg-yellow-500 text-white px-4 py-2 rounded-md hover:bg-yellow-600 transition"
            onClick={() => openCategoryModal("Update")}
          >
            Update Category
          </Button>
          <Button
            className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600 transition"
            onClick={() => openCategoryModal("Delete")}
          >
            Delete Category
          </Button>
        </div>
