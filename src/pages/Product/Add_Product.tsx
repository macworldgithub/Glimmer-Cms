...
const ProductPage = () => {
    ...
    const [sizes, setSizes] = useState([{ id: 1, value: "", unit: "mg" }]);
    const [types, setTypes] = useState([{ id: 1, value: "" }]);
    ...
    const removeSizeField = (id) => {
        setSizes((prevSizes) => prevSizes.length > 1 ? prevSizes.filter(size => size.id !== id) : [{ id: 1, value: "0", unit: "mg" }]);
    };

    const removeTypeField = (id) => {
        setTypes((prevTypes) => prevTypes.length > 1 ? prevTypes.filter(type => type.id !== id) : [{ id: 1, value: "0" }]);
    };

    const handleSizeChange = (id, key, value) => {
        setSizes((prevSizes) =>
            prevSizes.map((size) => (size.id === id ? { ...size, [key]: value } : size))
        );
                                ...
                                {sizes.map((size, index) => (
                                    <div key={size.id} className="flex items-center gap-2 mb-2">
                                        <input
                                            ...
                                            value={size.value}
                                            onChange={(e) => handleSizeChange(size.id, "value", e.target.value)}
                                        />
                                        <select
                                            className="p-3 border border-gray-400 rounded-md"
                                            value={size.unit}
                                            onChange={(e) => handleSizeChange(size.id, "unit", e.target.value)}
                                        >
                                            <option value="mg">mg</option>
                                            <option value="kg">kg</option>
                                            <option value="l">l</option>
                                        </select>
                                        <button onClick={() => removeSizeField(size.id)} className="ml-2">-</button>
                                    </div>
                                ))}
                                <button onClick={addSizeField} className="mt-2">+ Add Size</button>
                            </div>
                            <div className="mb-4">
                                ...
                                {types.map((type, index) => (
                                    <div key={type.id} className="mb-2">
                                        ...
                                        />
                                        <button onClick={() => removeTypeField(type.id)} className="ml-2">-</button>
                                    </div>
                                ))}
                                <button onClick={addTypeField} className="mt-2">+ Add Type</button>
                            </div>
                        </div>
                    </div>
                </div>
